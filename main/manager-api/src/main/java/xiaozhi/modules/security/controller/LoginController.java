...
public class LoginController {
    ...
    public Result<Void> smsVerification(@RequestBody SmsVerificationDTO dto) {
        ...
        if (!isMobileRegister) {
            throw new RenException(ErrorCode.MOBILE_REGISTER_NOT_ENABLED);
        }
        // 发送短信验证码
    ...
    public Result<TokenDTO> login(@RequestBody LoginDTO login) {
        ...
        if (!validate) {
            throw new RenException(ErrorCode.CAPTCHA_ERROR_INVALID);
        }
        // 按照用户名获取用户
        SysUserDTO userDTO = sysUserService.getByUsername(login.getUsername());
        // 判断用户是否存在
        if (userDTO == null) {
            throw new RenException(ErrorCode.USERNAME_PASSWORD_ERROR);
        }
        // 判断密码是否正确，不一样则进入if
    ...
    public Result<Void> register(@RequestBody LoginDTO login) {
        if (!sysUserService.getAllowUserRegister()) {
            throw new RenException(ErrorCode.USER_REGISTER_NOT_ALLOWED);
        }
        // 是否开启手机注册
        Boolean isMobileRegister = sysParamsService
                .getValueObject(Constant.SysMSMParam.SERVER_ENABLE_MOBILE_REGISTER.getValue(), Boolean.class);
        ...
        if (isMobileRegister) {
            ...
            if (!validPhone) {
                throw new RenException(ErrorCode.USERNAME_NOT_MOBILE);
            }
            // 验证短信验证码是否正常
            validate = captchaService.validateSMSValidateCode(login.getUsername(), login.getMobileCaptcha(), false);
            if (!validate) {
                throw new RenException(ErrorCode.MOBILE_CODE_ERROR);
            }
        } else {
            // 验证是否正确输入验证码
        ...
        SysUserDTO userDTO = sysUserService.getByUsername(login.getUsername());
        if (userDTO != null) {
            throw new RenException(ErrorCode.MOBILE_ALREADY_REGISTERED);
        }
    ...
    public Result<?> retrievePassword(@RequestBody RetrievePasswordDTO dto) {
        ...
        if (!validPhone) {
            throw new RenException(ErrorCode.MOBILE_FORMAT_INVALID);
        }

        // 按照用户名获取用户
        SysUserDTO userDTO = sysUserService.getByUsername(dto.getPhone());
        if (userDTO == null) {
            throw new RenException(ErrorCode.MOBILE_NOT_REGISTERED);
        }
        // 验证短信验证码是否正常
